...
public class GalleryLayoutSizeCalculator {
    ...
    public GalleryLayoutSizeCalculator(SizeCalculatorDelegate sizeCalculatorDelegate) {
    ...
    private void computeChildSizesUpToPosition(int lastPosition) {
        ...
        double minAspectRatio = Integer.MAX_VALUE;
        double minDiag = Integer.MAX_VALUE;
...
//        while (pos <= lastPosition || (currentRowHeight > mMaxRowHeight)) {
        while(pos <= lastPosition || !isRowFull) {
//        while (pos <= lastPosition || (mIsFixedHeight ? currentRowWidth <= mContentWidth : (minDiag > mMinDiag) && (currentRowHeight > mMaxRowHeight))) {
            ...
            if (posAspectRatio < minAspectRatio)
                minAspectRatio = posAspectRatio;

//            minDiag = Math.sqrt(Math.pow(calculateWidth(currentRowHeight, minAspectRatio), 2) +
//                    Math.pow(currentRowHeight, 2));

//            boolean isRowFull = mIsFixedHeight ? currentRowWidth > mContentWidth : (minDiag <= mMinDiag) || (currentRowHeight <= mMaxRowHeight);
            isRowFull = currentRowHeight <= mMaxRowHeight && itemAspectRatios.size() > 1;
            if (isRowFull) {

                minAspectRatio = Integer.MAX_VALUE;
